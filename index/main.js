...
const mercuryTexture = new THREE.TextureLoader().load("2k_mercury.jpg"); // Make sure this file path is correct
...
const venusTexture = new THREE.TextureLoader().load("2k_venus_surface.jpg"); // Update path if needed
...
const marsTexture = new THREE.TextureLoader().load("2k_mars.jpg"); // Adjust path if needed
...
const jupiterTexture = new THREE.TextureLoader().load("2k_jupiter.jpg"); // Adjust path if needed
...
const uranusTexture = new THREE.TextureLoader().load("2k_uranus.jpg"); // Adjust path if needed
...
const neptuneTexture = new THREE.TextureLoader().load("2k_neptune.jpg"); // Adjust path if needed
...
const saturnTexture = new THREE.TextureLoader().load("2k_saturn.jpg"); // Adjust path if needed
...
const sunTexture = new THREE.TextureLoader().load("2k_sun.jpg"); // Adjust path if needed
...
const moonTexture = new THREE.TextureLoader().load("2k_moon.jpg"); // Adjust path if needed
...
const ceresTexture = new THREE.TextureLoader().load("2k_ceres.jpg"); // Adjust texture path
...
const haumeaTexture = new THREE.TextureLoader().load("2k_haumea.jpg"); // Use Haumea texture
...
const makemakeTexture = new THREE.TextureLoader().load("2k_makemake.jpg"); // use a Makemake texture image
...
const erisTexture = new THREE.TextureLoader().load("2k_eris.jpg"); // Eris texture image
...
const earthTexture = new THREE.TextureLoader().load("2k_earth.jpg"); // Use realistic Earth texture
...
// earth-night
const earthNightContainer = document.getElementById("earth-night");
// Renderer
const earthNightRenderer = new THREE.WebGLRenderer({ antialias: true, alpha: true });
earthNightRenderer.setSize(200, 200);
earthNightContainer.appendChild(earthNightRenderer.domElement);

// Scene & Camera
const earthNightScene = new THREE.Scene();
const earthNightCamera = new THREE.PerspectiveCamera(45, 1, 0.1, 100);
earthNightCamera.position.z = 3;

// Texture (night side only)
const nightTexture = new THREE.TextureLoader().load("2k_earth_night.jpg"); // Use city lights texture
...
// Geometry
const earthNightGeometry = new THREE.SphereGeometry(1.1, 64, 64);
const earthNightMesh = new THREE.Mesh(earthNightGeometry, earthNightMaterial);
earthNightScene.add(earthNightMesh);

// Lights (dim to emphasize night lights)
const ambientLight = new THREE.AmbientLight(0xffffff, 0.1); // Low ambient light
const pointLight = new THREE.PointLight(0xffffff, 0.4); // Soft moonlight feel
pointLight.position.set(3, 3, 3);
earthNightScene.add(ambientLight, pointLight);

// Animation
function animateEarthNight() {
  requestAnimationFrame(animateEarthNight);
  earthNightMesh.rotation.y += 0.01;
  earthNightRenderer.render(earthNightScene, earthNightCamera);
}
animateEarthNight();
...
const cloudTextureMap = earthCloudsTextureLoader.load("2k_earth_clouds.jpg"); // Transparent PNG clouds
